...
from src.schemas.bookings import BookingDTO, BookingAddDTO, BookingAddRequestDTO
...
class BookingService(BaseService):
    ...
    async def get_user_bookings(self, user_id: int) -> list[BookingDTO]:
        return await self.db.bookings.get_filtered(user_id=user_id)  # type: ignore

    async def add_booking(self, user_id: int, booking_data: BookingAddDTO) -> BookingDTO:
        ...
        _booking_data = BookingAddRequestDTO(
