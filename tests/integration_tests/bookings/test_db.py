...
async def test_booking_crud(db: DBManager):
    ...
    # read data
    read_booking_data = (await db.bookings.get_all())[-1]
    assert read_booking_data.user_id == user_id
    assert read_booking_data.room_id == room_id
    assert read_booking_data.date_from == date(year=2025, month=12, day=12)
    assert read_booking_data.date_to == date(year=2025, month=12, day=22)
